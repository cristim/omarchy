...
When something asks for root through `pkexec`, the password prompt shows the command it wants to run and which programs asked for it, as in _Not verified: requested by omarchy-update ← bash ← foot_. That chain is read locally from the waiting process, and a program can name itself anything, so treat it as a hint rather than proof.

Press `Tab` (or click the hint) to ask your default agent what the command is likely to do. Its answer appears in the prompt as that agent's guess, so read it as a second opinion, not a verdict. Nothing is sent until you ask; then the command line, the programs that asked, and the prompt's title go to that agent's provider. Since whatever wants root wrote that command line, the agent treats it as untrusted text and answers without any tool that can read, run, or change anything. It also runs in a sandbox that hides your files and your session's sockets and passes on only the environment variables agents use. The agent gets a copy of its own login, and a login it refreshes is saved back. It keeps network access to reach its provider, which also reaches services on your machine that listen on the network. Claude Code, Codex, and Pi can be run that way; the other agents can't be kept from using tools, so the hint doesn't appear for them. You can ask the same question from a terminal with `omarchy agent explain "<command>"`.
